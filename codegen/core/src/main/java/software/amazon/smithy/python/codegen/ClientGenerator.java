...
final class ClientGenerator implements Runnable {
        ...
        writer.openBlock("class $L:", "", serviceSymbol.getName(), () -> {
            ...
            writer.writeDocs(() -> {
                writer.write("""
                        $L
                        Args:
                            config: Optional configuration for the client. Here you can set things like the
                                endpoint for HTTP services or auth credentials.
                            plugins: A list of callables that modify the configuration dynamically. These
    ...
    private void writeSharedOperationInit(PythonWriter writer, OperationShape operation, Shape input) {
        writer.writeDocs(() -> {
            ...
            writer.write("""
                    Args:
                        plugins: A list of callables that modify the configuration dynamically.
